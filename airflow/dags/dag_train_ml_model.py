...
def train_model(
    ...
    logger.info(f"The DictVectorizar has {len(dv.feature_names_)} features")
    
    lr = LinearRegression()
    lr.fit(X_train, y_train)
    y_pred = lr.predict(X_train)
    rmse = mean_squared_error(y_train, y_pred, squared=False)
    logger.info(f"Tle RMSE of train is: {rmse}")
    return lr, dv, rmse
...
def taskflow():
    ...
    def buid_machine_learning_model(paths: Dict[str,str]):
    ...
    buid_machine_learning_model(get_paths())
